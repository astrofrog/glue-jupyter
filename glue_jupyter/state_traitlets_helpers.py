...
def update_state_from_dict(state, changes):
    ...
    # Determine which properties actually differ from the current state
    # BEFORE making any changes. This prevents callbacks triggered by
    # high-priority properties from making lower-priority properties
    # appear changed. For example, toggling x_log resets x_min/x_max
    # via a callback, and without pre-filtering, stale x_min/x_max
    # values from the browser would be applied because they now differ
    # from the callback-updated values.
    actual_changes = {}
    for name in changes:
